...
def write_csv(path: Path, rows: List[dict]):
    ...
    # Some outputs mix multiple record types (for example OOF confusion rows and
    # branch_debug rows), so different rows may not share the exact same keys.
    # Build the CSV header as the ordered union of all keys and backfill missing
    # fields with empty strings to avoid DictWriter field mismatch errors.
    keys = []
    seen = set()
    for row in rows:
        for k in row.keys():
