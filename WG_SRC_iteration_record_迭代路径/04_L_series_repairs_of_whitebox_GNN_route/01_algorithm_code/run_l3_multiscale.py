...
def main():
    ...
    log(f"L3 Multi-Scale GCN | Cora | hidden={HIDDEN_DIM} hops={NUM_HOPS} "
        f"lambda_mcr={LAMBDA_MCR} warmup={MCR_WARMUP}")
    log("=" * 70)
    ...
    out_path = os.path.join(OUT_DIR, "l3_ms_output.txt")
    ...
    delta_val  = best_val  - g1_val
    delta_test = best_test - g1_test
    result_lines = [
        "L3 Multi-Scale GCN Result",
        "=" * 40,
        f"Best Val  Acc: {best_val:.4f}",
        f"Best Test Acc: {best_test:.4f}",
        f"G1 Baseline:  val={g1_val}, test={g1_test}",
        f"Delta Val:  {delta_val:+.4f}",
        f"Delta Test: {delta_test:+.4f}",
        "=" * 40,
    ]
    res_path = os.path.join(OUT_DIR, "l3_ms_result.txt")
