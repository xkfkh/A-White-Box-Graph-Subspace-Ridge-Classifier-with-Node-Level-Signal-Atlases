"""
L2 Innovation: Cosine Tau Annealing + Hop Diversity Regularization (FIXED)
Fix: eps=0.5 -> eps=0.1 (coeff boost 25x), hidden_dim=64, subspace_dim=16
     patience=50, epochs=200, lambda_lap=0.1

Original eps=0.5: coeff = d/(n*eps^2) = 16/(2708*0.25) ~ 0.024 -> delta_R ~ 0.003, uniform hop weights
Fixed  eps=0.1: coeff = d/(n*eps^2) = 16/(2708*0.01) ~ 0.59  -> delta_R >> 0.003, distinguishable
...
Config: hidden=64, subspace=16, epochs=200, patience=50 (L2 fixed version)
"""
...
OUTPUT_PATH = os.path.join(OUT_DIR, "l2_tau_fixed_output.txt")
RESULT_PATH = os.path.join(OUT_DIR, "l2_tau_fixed_result.txt")
...
def coding_rate_gradient(Z, eps=0.1):
...
class CRGainGNNAnnealed(nn.Module):
    def __init__(self, in_dim, hidden_dim, num_classes, subspace_dim,
                 ...
                 lambda_lap=0.1, lambda_sparse=0.05, dropout=0.6):
        super().__init__()
        self.num_hops = num_hops
...
def run_experiment():
    ...
    log("=" * 60)
    log("Experiment: L2 Tau Annealing FIXED (eps=0.1, hidden=64, sub=16)")
    log(f"Config: {cfg}")
    log("=" * 60)
    ...
    # coeff diagnostic
    coeff_old = 16 / (n * 0.5 * 0.5)
    coeff_new = cfg['subspace_dim'] / (n * cfg['eps'] * cfg['eps'])
    log(f"coeff (eps=0.5): {coeff_old:.4f}  ->  coeff (eps=0.1, sub=16): {coeff_new:.4f}  (x{coeff_new/coeff_old:.1f})")
...
if __name__ == '__main__':
    log("L2 Innovation: Tau Annealing FIXED (eps=0.1, hidden=64, subspace=16)")
    ...
    result_lines = [
        "L2 Tau Annealing FIXED Result",
        "=" * 40,
        "Config: l2_tau_fixed",
        "  hidden_dim=64, subspace_dim=16, num_hops=2",
        "  eps=0.1 (was 0.5), epochs=200, patience=50",
        "  lambda_lap=0.1 (was 0.3)",
